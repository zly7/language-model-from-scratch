...
class cosFormerBlock(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.ln_1 = LayerNorm(config.n_embd, bias=config.bias)
        self.attn = CosformerAttention(embed_dim=config.n_embd, num_heads=config.n_head, dropout_rate=config.dropout,causal=True,batch_first=True)
        self.ln_2 = LayerNorm(config.n_embd, bias=config.bias)
        self.mlp = MLP(config)
...
class GPT(nn.Module):
    ...
    @classmethod
    def from_pretrained(cls, model_type, override_args=None,model_hf=None):
