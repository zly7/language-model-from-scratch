...
def main():
    ...
    #             n_layer=12, n_head=12, dropout=0.1, use_reformer=True,common_lsh_bucket_size=512,n_lsh_hash=1)
    # config = BertConfig(vocab_size=tokenizer.vocab_size, n_embd=768, 
    #     n_layer=12, n_head=12, dropout=0.1, use_reformer=True)
    config = BertConfig(vocab_size=tokenizer.vocab_size, n_embd=512, 
        n_layer=8, n_head=8, dropout=0.1, use_reformer=True)
    ...
    batch_size = get_batch_size("base","bert",sequence_length,use_reformer = True)
    all_batch_size = 256
    ...
    args = TrainingArgumentsSelf(
        output_dir=f"reformer_bert_pretrain/wa-{date_string}/",
