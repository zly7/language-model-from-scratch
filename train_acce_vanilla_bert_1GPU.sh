#!/bin/bash
#SBATCH -o job.%j-train-vanilla-bert-1gpu.out          # 脚本执行的输出将被保存在当job.%j.out文件下，%j表示作业号;
#SBATCH --partition=gpulab02      
#SBATCH --qos=gpulab02             # 指定作业的QOS
#SBATCH -J cos-bert-1GPU       # 作业在调度系统中的作业名为myFirstJob;
...
nvidia-smi
accelerate launch --config_file acce_config_1GPU.yaml train_vanilla_bert.py