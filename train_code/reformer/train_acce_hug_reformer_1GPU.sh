#!/bin/bash
#SBATCH -o job.%j-train-re-bert-2gpu.out          # 脚本执行的输出将被保存在当job.%j.out文件下，%j表示作业号;
...
nvidia-smi
accelerate launch --config_file acce_config_1GPU.yaml --main_process_port 29501 train_hug_reformer.py