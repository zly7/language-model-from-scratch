...
def main():
    ...
    #          hidden_size=768,num_buckets=32,max_position_embeddings=sequence_length,is_decoder=True)
    ### 下面的参数是完全按照谷歌的参数来的
    config = ReformerConfig(vocab_size=tokenizer.vocab_size, num_attention_heads=8,attention_head_size=128, 
             ...
             num_hashes=4,is_decoder=True,max_position_embeddings=sequence_length)
 
    from transformers import ReformerModelWithLMHead
    model = ReformerModelWithLMHead(config)
    model_size = sum(t.numel() for t in model.parameters())
    print(f"gpt-reformer size: {model_size/1000**2:.1f}M parameters")
    ...
    if config.hidden_size == 1024:
        ...
        gradient_ac = 8 # 这里注意会覆写gradient_ac
    ...
    args = TrainingArgumentsSelf(
        output_dir=f"hug_re_pretrain_chartoken_gpt/{date_string}/",
