...
def main():
    ...
    config = ReformerConfig(vocab_size=tokenizer.vocab_size, num_attention_heads=8,attention_head_size=128, 
             attn_layers=['local','local','lsh','local','local','local','lsh','local','local','local','lsh','local',],
            #  attn_layers=['lsh',],
             feed_forward_size=1024*4,axial_pos_shape=[32,64],axial_pos_embds_dim=[256,768],hidden_size=1024,num_buckets=32,
             whether_use_tree_attention=False,num_hashes=4,is_decoder=True)
    ...
    args = TrainingArgumentsSelf(
        output_dir=os.path.join(root_path,f"hug_re_pretrain_gpt/new_data-{date_string}/"),
