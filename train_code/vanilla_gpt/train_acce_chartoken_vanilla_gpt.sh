#!/bin/bash
#SBATCH -o job.%j-train-vanilla-gpt-chartoken-2gpu.out          # 脚本执行的输出将被保存在当job.%j.out文件下，%j表示作业号;
...
nvidia-smi
accelerate launch --config_file acce_config.yaml --main_process_port 29510 train_vanilla_gpt_char_token.py