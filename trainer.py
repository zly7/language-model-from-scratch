    ...
    def train(self):
        ...
        for epoch in range(self.args.num_train_epochs):
            ...
            for step, batch in enumerate(self.train_dataloader):
                ...
                # print("loss : " + str(loss))
                # loss.retain_grad()
                # loss = loss/ self.accelerator.gradient_accumulation_steps  # This code will be implemented in ACCELERATE，但是是另外一种形式
                if self.args.whether_hg_accelerator:
                    self.accelerator.backward(loss)
                else:
                    loss.backward()
                # if outputs.logits.grad is not None:
                #     print("outputs.logits.grad : " + str(torch.norm(outputs.logits.grad)))
