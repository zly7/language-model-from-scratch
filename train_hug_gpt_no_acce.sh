#!/bin/bash
#SBATCH -o job.%j-hug-gpt-no-acce.out          # 脚本执行的输出将被保存在当job.%j.out文件下，%j表示作业号;
#SBATCH --partition=gpulab02      
#SBATCH --qos=gpulab02             # 指定作业的QOS
#SBATCH -J zly-hug-gpt-no-acce       # 作业在调度系统中的作业名为myFirstJob;
...
nvidia-smi
python3 train_hug_gpt_no_acce.py